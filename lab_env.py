...
def init_state_lab(key, cfg, model,agent_params):
    ...
    agents = AgentState(
        position=random.randint(sk_pos, (cfg.n_agents_max, 2), 10, cfg.n-10),
...
def run_simulation_chunk_lab(state,model,keys, cfg):
...
# @partial(jax.jit, static_argnames=['cfg','model'])
def launch_one_agent_one_env(agent_params,key,cfg,model): 
    cfg = cfg._replace(
        n=30,
        n_agents_max=2,
    ...
    state = init_state_lab(key,cfg, model,agent_params)
    key, subkey = jax.random.split(key)
    keys_chunk = jax.random.split(subkey, 500)
    state, outputs = run_simulation_chunk_lab(state,model,keys_chunk, cfg)

    
    return state,outputs


def vmap_over_agents_env_lab(list_agents_param,key,model,cfg):
    return vmap(launch_one_agent_one_env,in_axes=(0,None,None,None))(list_agents_param,key,model,cfg)